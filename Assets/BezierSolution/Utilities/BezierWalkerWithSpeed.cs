...
namespace BezierSolution
{
    ...
    private void Awake()
      ...
      moveEmitter = GetComponent<FMODUnity.StudioEventEmitter>();
		...
		private void Update()
		{
      // Player crystal seeker, send progress to Miner Manager
      if (isGoingForward)
      {
        GameManager.minerManager.UpdateProgressPct((progress - startProgress) / (endProgress - startProgress)); 
      }

      // Enemy crystal seeker
      else
      {
        GameManager.bushManager.UpdateProgressPct((startProgress - progress) / (startProgress - endProgress));
      }
